...
export function PropertyCard({ house_list }: PropertyCardProps) {
  return (
    <Card className="overflow-hidden">
      <div className="relative">
        ...
        <Image
          src={house_list.image || "/placeholder.svg"}
          alt={house_list.name}
          width={500}
          height={300}
          loader={imageLoader}
      ...
      <CardContent className="p-4">
        <div className="mb-2">
          <RatingStars rating={house_list.star} reviews={house_list.star} />
        </div>
        <h3 className="mb-1 text-lg font-semibold">{house_list.name}</h3>
        <p className="mb-3 text-sm text-muted-foreground">
          {house_list.description}
        </p>
        <div className="flex items-start gap-2 text-sm text-muted-foreground">
          <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
          <span>{house_list.location}</span>
        </div>
      ...
      <CardFooter className="flex items-center justify-between border-t p-4">
        <div className="font-semibold text-primary">{house_list.price}</div>
        <Link href={`/properties/${house_list.house_id}`}>
